...
misingroup=$(cat $(grep -f $ingroupfile <(find ../../ -name "full_table*")) | grep -v "#" | grep -P "$ID" | grep -e "Missing" -e "Fragmented" | cut -f 1| wc -l)
...
misoutgroup=$(cat $(grep -f $outgroupfile <(find ../../ -name "full_table*")) | grep -v "#" | grep -P "$ID" | grep -e "Missing" -e "Fragmented" | cut -f 1| wc -l)
...
avg=$(for s in $(cat $ingroupfile $outgroupfile ); do cat $(find ../../ -name "full_table*" | grep -P "$s") | grep -v "#" | grep -P "$ID" | grep -v "Missing" | grep -v "Fragmented" | wc -l; done | perl -ne 'chomp; push @array, $_; if (eof()){$sum=0; for($i=0; $i<scalar(@array); $i++){$sum = $sum + $array[$i]}; $avg = $sum / scalar(@array); print "$avg\n"}')
...
#median
med=$(for s in $(cat $ingroupfile $outgroupfile ); do cat $(find ../../ -name "full_table*" | grep -P "$s") | grep -v "#" | grep -P "$ID" | grep -v "Missing" | grep -v "Fragmented" | wc -l; done | grep -v "^0$" | perl -ne 'chomp; push @array, $_; if (eof()){$len=@array; @sorted = sort {$a <=> $b} @array; print "Length: $len\t".join(" ", @sorted)."\n"; if ($len%2){print "$sorted[int($len/2)]\n"}else{$med = ($sorted[int($len/2)] + $sorted[int($len/2)-1])/2; print "$med\n"}}' | tail -n 1)
