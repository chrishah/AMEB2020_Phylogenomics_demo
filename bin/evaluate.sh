...
if [ "$misingroup" -lt 2 ] && [ "$misoutgroup" -lt 2 ]
then
...
#average
#avg=$(cat $(find ../../../genes/ -name "full_table*") | grep -v "#" | grep -P "$ID" | cut -f 1,2 | uniq -c | perl -ne 'chomp; @a=split(" "); if ($a[2] =~ /Duplicated/){push @array, $a[0]}else{for($i=0; $i<$a[0]; $i++){push @array, 1;}}; if (eof()){$sum=0; for($i=0; $i<scalar(@array); $i++){$sum = $sum + $array[$i]}; $avg = $sum / scalar(@array); print "$avg\n"}')
avg=$(for s in $(cat $ingroupfile $outgroupfile ); do cat $(find ../../../genes/ -name "full_table*" | grep -P "$s") | grep -v "#" | grep -P "$ID" | grep -v "Missing" | grep -v "Fragmented" | wc -l; done | perl -ne 'chomp; push @array, $_; if (eof()){$sum=0; for($i=0; $i<scalar(@array); $i++){$sum = $sum + $array[$i]}; $avg = $sum / scalar(@array); print "$avg\n"}')
...
#median
med=$(for s in $(cat $ingroupfile $outgroupfile ); do cat $(find ../../../genes/ -name "full_table*" | grep -P "$s") | grep -v "#" | grep -P "$ID" | grep -v "Missing" | grep -v "Fragmented" | wc -l; done | grep -v "^0$" | perl -ne 'chomp; push @array, $_; if (eof()){$len=@array; @sorted = sort {$a <=> $b} @array; print "Length: $len\t".join(" ", @sorted)."\n"; if ($len%2){print "$sorted[int($len/2)]\n"}else{$med = ($sorted[int($len/2)] + $sorted[int($len/2)-1])/2; print "$med\n"}}' | tail -n 1)
